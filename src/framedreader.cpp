...
void FramedReader::checkSettings()
{
    ...
    {
        _settingsWidget.showMessage(tr("Sync word is invalid!"), true);
    }
    else if (settingsInvalid & FRAMESIZE_INVALID)
    {
        QString errorMessage =
            QString(tr("Frame size must be multiple of %1 (#channels * sample size)!"))\
    ...
    {
        _settingsWidget.showMessage(tr("All is well!"));
    }
}
