    ...
    resetMenu(tr("Reset Menu"), this)
{
    ...
    connect(ui->spLineThickness, &QSpinBox::valueChanged,
            [this](int thickness)
            {
    ...
    for (int nbits = 8; nbits <= 24; nbits++) // signed binary formats
    {
        ...
        ui->cbRangePresets->addItem(
            QString(tr("Signed %1 bits %2 to +%3")).arg(nbits).arg(rmin).arg(rmax),
    ...
    for (int nbits = 8; nbits <= 24; nbits++) // unsigned binary formats
    {
        ...
        ui->cbRangePresets->addItem(
            QString(tr("Unsigned %1 bits %2 to +%3")).arg(nbits).arg(0).arg(rmax),
            QVariant::fromValue(Range{0, double(rmax)}));
    }
    ui->cbRangePresets->addItem(tr("-1 to +1"), QVariant::fromValue(Range{-1, +1}));
    ui->cbRangePresets->addItem(tr("0 to +1"), QVariant::fromValue(Range{0, +1}));
    ui->cbRangePresets->addItem(tr("-100 to +100"), QVariant::fromValue(Range{-100, +100}));
    ui->cbRangePresets->addItem(tr("0 to +100"), QVariant::fromValue(Range{0, +100}));
