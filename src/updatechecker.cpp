...
void UpdateChecker::onReqFinished(QNetworkReply* reply)
{
    ...
    {
        emit checkFailed(QString(tr("Network error: ")) + reply->errorString());
    }
    else
    {
        ...
        {
            emit checkFailed(QString(tr("JSon parsing error at %1: ")).arg(error.offset)
                             + error.errorString());
            qCritical() << data;
        }
        else
        {
            ...
            {
                emit checkFailed(tr("JSON parsing error."));
