    ...
    bpsLabel(&portControl, &dataFormatPanel, this)
{
    ...
    ui->tabWidget->insertTab(0, &portControl, tr("Port"));
    ui->tabWidget->insertTab(1, &dataFormatPanel, tr("Data Format"));
    ui->tabWidget->insertTab(2, &plotControlPanel, tr("Plot"));
    ui->tabWidget->insertTab(3, &commandPanel, tr("Commands"));
    ui->tabWidget->insertTab(4, &recordPanel, tr("Record"));
    ui->tabWidget->insertTab(5, &textView, tr("Text View"));
    ...
    langGroup->addAction(ui->actionLangZh);
    langGroup->addAction(ui->actionLangEn);

    QString curLang = QSettings(PROGRAM_NAME, PROGRAM_NAME).value("Language").toString().toLower();
    if (curLang.isEmpty())
    {
        curLang = (QLocale::system().language() == QLocale::Chinese) ? "zh_cn" : "en";
    }
    if (curLang.startsWith("zh"))
        ui->actionLangZh->setChecked(true);
    else
        ui->actionLangEn->setChecked(true);

    connect(ui->actionLangZh, &QAction::triggered, this, [this](){ setLanguage("zh_cn"); });
    connect(ui->actionLangEn, &QAction::triggered, this, [this](){ setLanguage("en"); });

    // init view menu
    ui->menuBar->insertMenu(ui->menuSecondary->menuAction(), &plotMenu);
    plotMenu.addSeparator();
    QMenu* tbMenu = plotMenu.addMenu(tr("Toolbars"));
...
void MainWindow::closeEvent(QCloseEvent * event)
{
    ...
    if (!snapshotMan.isAllSaved())
    {
        auto clickedButton = QMessageBox::warning(
            this, tr("Closing SerialPlot"),
            tr("There are un-saved snapshots. If you close you will loose the data."),
    ...
    if (settings.status() != QSettings::NoError)
    {
        ...
        if (settings.status() == QSettings::AccessError)
        {
            ...
            errorText = QString(tr("Serialplot cannot save settings due to access error. \
...
Try fixing the permissions of file: %1, or just delete it.")).arg(file);
        ...
        {
            errorText = QString(tr("Serialplot cannot save settings due to unknown error: %1")).\
                arg(settings.status());
        }

        auto button = QMessageBox::critical(
            NULL,
            tr("Failed to save settings!"), errorText,
...
void MainWindow::onSpsChanged(float sps)
{
    ...
    spsLabel.setText(QString::number(sps, 'f', precision) + tr("sps"));
}
...
{
    QSettings(PROGRAM_NAME, PROGRAM_NAME).setValue("Language", lang);
    QMessageBox::information(this, tr("Language"),
        tr("The language setting will take effect after restart."));
}
...
void MainWindow::handleCommandLineOptions(const QCoreApplication &app)
{
    ...
    parser.setApplicationDescription(tr("Small and simple software for plotting data from serial port in realtime."));
    parser.addHelpOption();
    parser.addVersionOption();

    QCommandLineOption configOpt({"c", "config"}, tr("Load configuration from file."), "filename");
    QCommandLineOption portOpt({"p", "port"}, tr("Set port name."), "port name");
    QCommandLineOption baudrateOpt({"b" ,"baudrate"}, tr("Set port baud rate."), "baud rate");
    QCommandLineOption openPortOpt({"o", "open"}, tr("Open serial port."));
